...
async function fetchWrapped(input: string, init: RequestInit) {
  try {
    ...
    if (serverName)
      throw new Error(
        `${serverName} is not responding. Please check your internet connection. If the problem persists, feel free email us at support@notesfriend.app. (Reference error: ${
          (e as Error).message
        })`
      );
