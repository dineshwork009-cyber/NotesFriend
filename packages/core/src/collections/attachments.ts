...
import { Cipher, DataFormat, SerializedKey } from "@notesfriend/crypto";
      ...
      types.includes("all")
        ...
        : selector.filter.where((eb) => {
            ...
            if (types.includes("webclips"))
              filters.push(
                eb("mimeType", "==", `application/vnd.notesfriend.web-clip`)
              );
    ...
    return this.collection.createFilter<Attachment>(
      (qb) =>
        qb
          ...
          .where("mimeType", "==", `application/vnd.notesfriend.web-clip`),
      this.db.options?.batchSize
    );
  }
...
export function getOutputType(attachment: Attachment): DataFormat {
  if (attachment.mimeType === "application/vnd.notesfriend.web-clip")
    return "text";
