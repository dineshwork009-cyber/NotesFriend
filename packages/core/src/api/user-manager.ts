...
import { SerializedKeyPair, SerializedKey, Cipher } from "@notesfriend/crypto";
...
class UserManager {
        ...
        await http
          .post(
            `${constants.AUTH_HOST}${ENDPOINTS.token}`,
            {
              ...
              client_id: "notesfriend",
              scope: "notesfriend.sync offline_access IdentityServerApi",
              password: hashedPassword
            },
            token.access_token
          )
  ...
  async verifyPassword(password: string) {
    try {
      ...
      const cipher = await this.db.storage().encrypt(key, "notesfriend");
