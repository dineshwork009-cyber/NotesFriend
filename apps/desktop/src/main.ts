...
import { Messages, setI18nGlobal } from "@notesfriend/intl";
...
if (process.platform == "win32" && process.env.PORTABLE_EXECUTABLE_DIR) {
  ...
  const root = path.join(process.env.PORTABLE_EXECUTABLE_DIR, "Notesfriend");
...
async function createWindow() {
  ...
  mainWindow.webContents.session.setSpellCheckerDictionaryDownloadURL(
    "http://dictionaries.notesfriend.com/"
  );
...
app.once("ready", async () => {
  ...
  if (app.runningUnderARM64Translation) {
    ...
    dialog.showMessageBoxSync({
      message:
        "Notesfriend detected that it is running under ARM64 translation. For the best performance, please download the ARM64 build of Notesfriend from our website.",
      type: "warning",
      buttons: ["Okay"],
