...
import { isReminderActive, Reminder } from "@notesfriend/core";
...
const onEvent = async ({ type, detail }: Event) => {
  ...
  if (
    ...
    Platform.OS === "android" &&
    notification?.id === "notesfriend_note_input" &&
...
async function updateRemindersForWidget() {
  ...
  NotesfriendModule.updateReminderWidget();
}
...
async function loadNote(id: string, jump: boolean) {
  if (!id || id === "notesfriend_note_input") return;
...
async function getChannelId(id: "silent" | "vibrate" | "urgent" | "default") {
  ...
  switch (id) {
    ...
    case "silent":
      return await notifee.createChannel({
        id: "com.streetwriters.notesfriend.silent",
        name: "Silent",
        vibration: false
      });
    case "vibrate":
    ...
    case "urgent":
      return await notifee.createChannel({
        id: "com.streetwriters.notesfriend.urgent",
        name: "Urgent",
        description:
...
async function pinQuickNote() {
  ...
  get().then((items) => {
    const notification = items.find((n) => n.id === "notesfriend_note_input");
