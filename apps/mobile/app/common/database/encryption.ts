...
import { SerializedKey } from "@notesfriend/crypto";
...
const IOS_KEYCHAIN_ACCESS_GROUP = "group.org.streetwriters.notesfriend";
const IOS_KEYCHAIN_SERVICE_NAME = "org.streetwriters.notesfriend";
const KEYCHAIN_SERVER_DBKEY = "notesfriend:db";

const NOTESFRIEND_APPLOCK_KEY_SALT = "kBwr1Kre86ebOZ8ThLu2OA";
const NOTESFRIEND_DB_KEY_SALT = "SNuzOcEK3amoqL0WvPeKqw";
...
export async function getDatabaseKey(appLockPassword?: string) {
  ...
  if (await Keychain.hasInternetCredentials("notesfriend")) {
    const userKeyCredentials =
      await Keychain.getInternetCredentials("notesfriend");

    if (userKeyCredentials) {
      const userKeyCipher: Cipher = (await encrypt(
        {
          key: DB_KEY,
          salt: NOTESFRIEND_DB_KEY_SALT
        },
        userKeyCredentials.password
      )) as Cipher;
      // Store encrypted user key in MMKV
      MMKV.setMap(USER_KEY_CIPHER, userKeyCipher);
      await Keychain.resetInternetCredentials("notesfriend");
    }
