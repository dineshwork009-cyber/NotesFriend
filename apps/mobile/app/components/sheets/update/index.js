...
export const Update = ({ version: appVersion, fwdRef }) => {
  ...
  let notes = version?.notes
    ? version.notes.replace("Thank you for using Notesfriend!", "").split("- ")
  ...
  const GITHUB_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/streetwriters/notesfriend/releases/download/${
          version.version
        }-android/notesfriend-${getSupportedAbi()}.apk`;
  const GITHUB_PAGE_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/streetwriters/notesfriend/releases/tag/${version.version}-android`;
