import { Plan, SubscriptionPlan, SubscriptionPlanId } from "@notesfriend/core";
...
const pricingPlans: PricingPlan[] = [
  ...
  },
  {
    ...
    productSkuList: ["notesfriend.pro.5year"],
    trialSupported: true,
    recommended: true
  },
  {
    ...
    productSkuList: ["notesfriend.believer.5year"],
    trialSupported: true
  }
];
...
const usePricingPlans = (options?: PricingPlansOptions) => {
  ...
  const [selectedProductSku, setSelectedProductSku] = useState<string>(
    options?.productId || "notesfriend.pro.yearly"
  );
  ...
  async function getRegionalDiscount(plan: string, productId: string) {
    if (productId !== "notesfriend.pro.yearly") {
      return;
    }
    try {
