...
const onAppOpenedFromURL = async (event: {
  ...
  try {
    if (url.startsWith("https://app.notesfriend.com/account/verified")) {
      ...
      return;
    } else if (url.startsWith("https://app.notesfriend.com/open_note?")) {
      ...
      url.startsWith("https://app.notesfriend.com/open_notebook?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
      ...
      url.startsWith("https://app.notesfriend.com/open_tag?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
      ...
      url.startsWith("https://app.notesfriend.com/open_color?") &&
      !event.isInitialUrl
    ) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
    ...
    } else if (url.startsWith("https://app.notesfriend.com/open_reminder")) {
      const id = new URL(url).searchParams.get("id");
      if (id) {
        const reminder = await db.reminders.reminder(id);
        if (reminder) AddReminder.present(reminder);
      }
    } else if (url.startsWith("https://app.notesfriend.com/new_reminder")) {
...
async function saveEditorState() {
  if (!editorState().movedAway) {
    ...
    NotesfriendModule.setAppState(state);
  } else {
