...
const Tests = {
  awaitLaunch: async () => {
    ...
    await waitFor(element(by.id(notesfriend.ids.default.root)))
      .toBeVisible()
      //@ts-ignore
  ...
  async createNote(title?: string, _body?: string) {
    ...
    await Tests.fromId(notesfriend.buttons.add).tap();
    if (title) {
  ...
  async navigate(screen: RouteName | ({} & string)) {
    let menu = Tests.fromId(notesfriend.ids.default.header.buttons.left);
    await menu.waitAndTap();
    await Tests.fromText(screen as string).waitAndTap();
  },
  async openSideMenu() {
    await Tests.fromId(notesfriend.ids.default.header.buttons.left).waitAndTap();
  },
  async prepare() {
    await device.disableSynchronization();
  ...
  async createNotebook(title = "Notebook 1", description = true) {
    ...
    const titleInput = Tests.fromId(
      notesfriend.ids.dialogs.notebook.inputs.title
    );
