...
mergedConfig.resolver = {
  ...
  nodeModulesPaths,
  extraNodeModules: {
    "@notesfriend": path.join(__dirname, "../../packages"),
  ...
  resolveRequest: (context, moduleName, platform) => {
    ...
    if (moduleName === "@notesfriend/core") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@notesfriend/common") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@notesfriend/logger") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }

    if (moduleName === "@notesfriend/theme") {
      const result = require.resolve(moduleName); // gets CommonJS version
      return context.resolveRequest(context, result, platform);
    }
