...
In Notesfriend, one note can belong to multiple notebooks. This allows for very flexible organization structures. For example, an author can "link" the character list to all the chapters instead of duplicating it for each chapter. Similarly, a user can link common notes between multiple notebooks without any duplication.
...
5. You have created your first notebook in Notesfriend
...
> info
>
> In Notesfriend a single note can exist in multiple Notebooks. However, a note will show only one reference on top.
